...
void main(){
    ...
    time(&t);
    printf("Welcome to Personal Diary with a lot of features\n");
    printf("Todays's date and time->%s",ctime(&t));//current time and date printed
    printf("Please enter your user name\n");
    scanf("%s",&name);
    xyz://part of goto statement
    printf("Enter 1 to make an account and 2 to login into your account\n");
    ...
    printf("\nWe have several options for you in the diary that are as follows\n");
    printf("\n\n");
...
void reminders(){//reminders and trips plan can be seen here
    ...
    if(ptr == NULL)
        ...
        printf("Your detailed meetings or plans are as follows:\n");
        c = fgetc(ptr);
        while((!feof(ptr))){
...
void birthdays(){//Usr can enter birthdays to remember
    printf("\nEnter birthdays\n");
    char b[2],c;
    int key = 3;
    fgets(b,2,stdin);
...
void add_passwords(){ //Passwords can be added by the user
    printf("Here you can add your important passwords that you want to remember\n");
    FILE *ptr,*fp;
    char pass[100000],c;
    int key = 3;
    ptr =fopen("E.txt","w");
...
void first(){//This function is to give user the available options
    int option =0;
    ...
    while(option != -1){
        ...
        if(option == -1){//If user enters -1 then code terminates
            printf("\nBye and thanks for using the diary\n");
            exit(0);
